...
"""
Detect Bitrix bitrixsetup.php files left in the webroot.
"""


CHECK_ID = "bitrix.setup.detect"
CHECK_NAME = "Detect Bitrix bitrixsetup.php exposure"

DEPENDS_ON = []
REQUIRES_DETECTED = []

REFERENCE_SETUP_MD5 = "ea204c68fabcec687fbb942a86021525"
...
SETUP_MARKERS = [
    ...
    "New version of bitrixsetup script is available!",
    "class CArchiver",
    "function LoaderGetMessage",
]


def build_scan_dirs(webroot) -> list:
    """
    Build a unique list of shallow scan directories from the webroot.
    """
...
def build_setup_scan_command(webroot) -> str:
    """
    Build a remote shell command that detects bitrixsetup.php signatures.
    """

    quoted_webroot = shlex.quote(webroot.rstrip("/"))
    quoted_reference_md5 = shlex.quote(REFERENCE_SETUP_MD5)
...
print_candidates | sort -u | while IFS= read -r file_path; do
    ...
    if [ "$file_name" = "bitrixsetup.php" ]; then
...
def parse_setup_scan_output(output) -> list:
    """
    Parse tab-separated bitrixsetup scan output from the remote shell.
    """
...
def format_reason(reasons) -> str:
    """
    Format bitrixsetup detection reasons for stdout.
    """
    ...
    labels = {
        "name": "filename bitrixsetup.php",
        "md5": "reference MD5",
        "marker": "bitrixsetup.php content marker",
    }
...
def format_findings(findings) -> str:
    """
    Format bitrixsetup.php findings for terminal and report output.
    """
...
def run(client, audit_config, ssh_config, context) -> dict:
    """
    Search selected remote webroot directories for bitrixsetup.php files.
    """
    ...
    command = build_setup_scan_command(webroot)
    ...
    findings = parse_setup_scan_output(scan_result["stdout"])
    detected = len(findings) > 0

    if "bitrix" not in context:
        context["bitrix"] = {}

    context["bitrix"]["bitrixsetup_php"] = {
        "detected": detected,
        "webroot": webroot,
        "reference_md5": REFERENCE_SETUP_MD5,
    ...
    if detected:
        result["stdout"] = (
            f"{red('[!] Possible file bitrixsetup.php found, double check and remove it!')}\n"
        ...
        result["stdout"] = (
            "Bitrix bitrixsetup.php file was not detected.\n"
            f"Remote webroot: {webroot}\n"
            f"Reference bitrixsetup.php MD5: {REFERENCE_SETUP_MD5}\n"
